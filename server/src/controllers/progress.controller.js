...
export const updateProgress = asyncHandler(async(req,res)=>{
    ...
    if(!progress){
        ...
        const lectureIndex = progress.completedLectures.find(lecture=>lecture.lectureId.toString()=== lectureId);
        if (lectureIndex !== -1) {
        // Update existing lecture progress
        progress.completedLectures[lectureIndex].watchedDuration = watchedDuration;
...
export const getCourseProgress = async (req, res) => {
  try {
    ...
    const progress = await Progress.findOne({ userId, courseId })
      .populate("completedLectures.lectureId");
