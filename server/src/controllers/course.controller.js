...
export const getCourseById = asyncHandler(async(req,res)=>{
    ...
    const course = await Course.findById(courseId).populate('instructorId', 'fullName email').populate('sections');
    if(!course){
        throw new ApiError(404, "Course not found");
    }
    return res.status(200).json({
...
export const getInstructorCourses = asyncHandler(async(req,res)=>{
    ...
    const courses = await Course.find({instructorId}).populate('instructorId', 'fullName email');
