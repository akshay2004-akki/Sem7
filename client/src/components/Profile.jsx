...
import { Mail, Calendar, Edit, BookOpen, Award, Settings, Save } from 'lucide-react';
...
export default function UserProfile() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await axios.get('http://localhost:8000/api/v1/users/profile', { withCredentials: true });
  ...
  if (!user) {
    return <div className="text-center text-gray-400 mt-32">Loading profile...</div>;
  }
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put('/api/users/update', { fullName: user.fullName, email: user.email }, { withCredentials: true });
    ...
    <div className="bg-black min-h-screen text-gray-300 font-sans p-4 sm:p-6 lg:p-8 pt-32">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 text-center mt-15">
          ...
          <p className="text-gray-400 mt-2">Manage your courses, wishlist, and account settings.</p>
        </div>
            ...
            <div className="relative group">
              <div className="relative bg-black p-6 rounded-2xl border border-zinc-800 text-center">
                ...
                <p className="text-xs text-gray-500 flex items-center justify-center gap-2 mt-2">
                  <Calendar size={14} /> Joined {new Date(user.createdAt).toDateString()}
                </p>
            ...
            <div className="bg-black rounded-2xl border border-zinc-800 h-full">
              ...
              <div className="border-b border-zinc-700">
                <nav className="flex space-x-2 p-2">
                  ...
                  <button onClick={() => setActiveTab('wishlist')} className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md ${activeTab === 'wishlist' ? 'bg-zinc-800 text-white' : 'text-gray-400 hover:bg-zinc-800/50'}`}>
                ...
                {activeTab === 'courses' && (
                  <div className="space-y-4">
                    ...
                    {user.enrolledCourses.map(course => (
                      <div key={course._id} className="bg-zinc-800 p-4 rounded-lg">
                ...
                {activeTab === 'settings' && (
                  <div className="space-y-10">
                    ...
                    <div className="border-t border-zinc-700 pt-6">
                      <h3 className="text-lg font-semibold text-white mb-4">Change Avatar</h3>
