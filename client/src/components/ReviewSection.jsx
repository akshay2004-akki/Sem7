...
const starRating = ({rating})=>{
    ...
    for(let  i = 0 ; i<fullStars ; i++){
        stars.push(<Star key={`full-${i}`} className="text-amber-400 fill-amber-400" size={20} />)
    }

    // for halfstar if there
    if(hasHalfStar){
        stars.push(<StarHalf key= 'half' className="text-amber-400 fill-amber-400" size={20} />)
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center">
          ...
          <p className="mt-4 max-w-2xl mx-auto text-lg text-gray-400">
            Hear from our community of learners and see how our platform has helped them achieve their goals.
          </p>
        </div>
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
            {reviewData.map((review) => (
              ...
              <div key={review.id} className="relative group transform transition-transform duration-300 hover:scale-105">
                {/* Gradient border that appears on hover */}
                <div className="absolute -inset-px bg-gradient-to-r from-cyan-400 via-pink-500 to-purple-500 rounded-xl opacity-0  group-hover:opacity-100 transition-opacity duration-500" aria-hidden="true" />
                  ...
                  <div className="flex items-start gap-5">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-4 text-gray-300 leading-relaxed relative">
                        <span className="absolute -top-3 -left-4 text-6xl text-gray-700 font-serif opacity-50">“</span>
                        {review.review}
                      </p>
